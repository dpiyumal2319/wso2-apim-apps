...
class API extends Resource {
    ...
    addGatewayEnvironment(name, displayName, type, description, gatewayType, mode, apiDiscoveryScheduledWindow, vhosts,
        permissions, additionalProperties, provider="wso2", planMappings = [], callback = null) {
        return this.client.then((client) => {
            const data = {
                name, displayName, type, description, gatewayType, mode, apiDiscoveryScheduledWindow, vhosts,
                permissions, additionalProperties, provider, planMappings,
            };
            const payload = {
                'Content-Type': 'application/json',
            };
    ...
    updateGatewayEnvironment(id, name, displayName, type, description, gatewayType, mode, apiDiscoveryScheduledWindow,
        vhosts, permissions, additionalProperties, provider="wso2", planMappings = [], callback = null) {
        return this.client.then((client) => {
            const data = {
                name, displayName, type, description, gatewayType, mode, apiDiscoveryScheduledWindow, vhosts,
                permissions, additionalProperties, provider, planMappings,
